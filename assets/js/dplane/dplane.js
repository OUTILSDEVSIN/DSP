// ============================================================
// DPLANE v5.1 -- fix décalage date 24h (toLocaleDateString)
// ============================================================
...
// ✅ FIX décalage 24h : utilise l'heure locale (YYYY-MM-DD) au lieu de toISOString() qui est en UTC
function dplaneDateStr(date) {
...
// ✅ FIX "aujourd'hui" : même logique locale pour la comparaison
function dplaneTodayStr() {
  return dplaneDateStr(new Date());
}
...
async function renderDplaneGrille() {
  ...
  // ✅ FIX : utilise dplaneTodayStr() au lieu de dplaneDateStr(new Date())
  const today=dplaneTodayStr(), role=dplaneGetRole(), canEdit=role==='admin'||role==='manager';
...
async function dplaneAjouterAbsenceUI(gestionnaireId, jour, creneau) {
  ...
  // ✅ FIX : +T12:00:00 pour éviter le décalage UTC lors du new Date()
...
async function dplaneCreerActivite() {
  ...
  const codeUnique = code + '_' + Date.now().toString(36);
