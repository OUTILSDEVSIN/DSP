// ============================================================
// DVOL v2.0 — Gestion des dossiers vol de véhicule
// Modale détail + suivi étapes + docs JSONB
// ============================================================

let dvolDossiers = [];
let dvolEtapesTemplate = [];

// ── Définition des 6 documents attendus ──
const DVOL_DOCS = [
...
// ── Lire la liste de docs depuis le JSONB documents_recus_liste ──
...
function dvolGetAlertes(dossiers) {
  ...
  for (const d of dossiers) {
    ...
    if (jours >= 10 && !d.date_lancement_expertise && !d.vehicule_retrouve_confirme) {
      alertes.push({ dossier: d, type: 'expertise_a_lancer',
        message: `J+${jours} — Expertise à lancer`,
        detail: `${d.numero_dossier} · ${d.assure_nom || '—'} · ${d.compagnie}`,
        urgence: jours >= 30 ? 'haute' : jours >= 20 ? 'moyenne' : 'normale' });
    }
    if (!d.documents_recus && jours >= 5 && docsManquants > 0) {
      alertes.push({ dossier: d, type: 'docs_manquants',
        message: `Documents manquants (${docsManquants}/6)`,
        detail: `${d.numero_dossier} · ${d.assure_nom || '—'} · J+${jours}`,
    ...
    if (d.date_lancement_expertise && !d.rapport_expertise_recu && !d.vehicule_retrouve_confirme) {
      ...
      if (jExp >= 7) {
        alertes.push({ dossier: d, type: 'rapport_manquant',
    ...
    if (d.vehicule_retrouve_confirme && d.statut === 'vehicule_retrouve') {
      alertes.push({ dossier: d, type: 'vehicule_retrouve',
        message: 'Véhicule retrouvé — expertise terrain à planifier',
        detail: `${d.numero_dossier} · ${d.assure_nom || '—'}`,
        urgence: 'haute' });
    }
  }
...
const DVOL_ALERTE_ICONS  = { expertise_a_lancer:'🔍', docs_manquants:'📄', rapport_manquant:'📋', vehicule_retrouve:'🚗' };
...
function dvolRenderTableau(dossiers) {
  ...
  if (dossiers.length === 0) {
    tbody.innerHTML = `<tr><td colspan="9" style="text-align:center;padding:40px;color:#9ca3af;"><div style="font-size:24px;margin-bottom:8px;">📂</div>Aucun dossier VOL pour le moment</td></tr>`;
    return;
  }
...
async function dvolOuvrirDossier(id) {
  ...
  // Charger le suivi des étapes pour ce dossier
  ...
  <div style="background:white;border-radius:16px;box-shadow:0 24px 64px rgba(0,0,0,.2);width:100%;max-width:760px;overflow:hidden;">
    ...
    <div style="background:linear-gradient(135deg,var(--navy,#1a2e4a),#2a4a6e);color:white;padding:20px 24px;display:flex;align-items:center;justify-content:space-between;gap:12px;">
      <div>
        <div style="font-size:18px;font-weight:800;">${d.numero_dossier}</div>
        <div style="font-size:13px;opacity:.8;margin-top:2px;">${d.assure_nom||'Assuré inconnu'} · ${d.compagnie_mere||d.compagnie||'—'} · ${dvolBadgeJours(jours)}</div>
      </div>
    ...
    <div style="padding:20px 24px;display:flex;flex-direction:column;gap:20px;">
      ...
      <button onclick="dvolSauvegarderEdition('${id}')" class="btn btn-primary" style="width:100%;padding:11px;">💾 Enregistrer les modifications</button>
      ` : ''}
