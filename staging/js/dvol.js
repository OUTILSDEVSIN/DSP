...
// Mise à jour : exploite les nouveaux champs de la vue
// dvol_tableau_de_bord (portefeuille, notes, action_requise,
...
async function dvolVerifierRelances(gestionnaireId) {
  ...
  for (const dossier of tableau) {
    if (gestionnaireId && dossier.gestionnaire_id !== gestionnaireId) continue;
    ...
    // Nouveau : action_requise calculée côté vue
    if (dossier.action_requise && !dossier.relance_cloture_active && !(dossier.date_prochaine_etape && dossier.date_prochaine_etape <= aujourd_hui)) {
      alertes.push({
        type: 'action',
        dossierId: dossier.id,
        numeroDossier: dossier.numero_dossier,
        assureNom: dossier.assure_nom,
        message: `⚡ Action requise — Dossier ${dossier.numero_dossier} (${dossier.assure_nom})`
      });
    }
  }
